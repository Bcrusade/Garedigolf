	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && ( empty($sponsors) || (!isset($settings['sponsors_status']) || !$settings['sponsors_status']) ) ) {
			...
			$html = '<div class="mec-content-notification"><p>'
					.'<span>'. esc_html__('This widget is displayed if sponsor is set. In order for the widget in this page to be displayed correctly, please set sponsor for your last event.', 'mec').'</span>'
					. '<a href="https://webnus.net/dox/modern-events-calendar/sponsor/" target="_blank">' . esc_html__('How to set sponsor', 'mec') . ' </a>'
				.'</p></div>';
