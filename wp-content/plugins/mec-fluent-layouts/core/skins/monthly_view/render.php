...
<dl class="mec-calendar-row mec-more-events-controller">
    ...
    for ($list_day = 1; $list_day <= $days_in_month; $list_day++) {
        ...
        if (isset($events[$today]) and count($events[$today])) {
            echo '<dt class="mec-calendar-day'.$selected_day.'" data-mec-cell="'.$day_id.'" data-day="'.$list_day.'" data-month="'.date('m', $time).'" data-year="'.date('Y', $time).'"><div class="'.$selected_day_date.'">'.$list_day.'</div>';
            $eventItems = 0;
            $moreEventsHTML = '';
            foreach ($events[$today] as $event) {
                ...
                if ($eventItems >= 2) {
                    ...
                    $title = null;
                    $class = 'event-single-link-simple';
                    $attributes = 'style="color: '.esc_attr($event_color).';"';
                    ...
                    echo '<h4 class="mec-event-title">'.$link.'</h4>';
                    echo '</div>';
                }
